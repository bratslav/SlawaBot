...
def main_menu(message,tekUser, text='Ласкаво просимо до нашого telegram bot'):
    if tekUser == -1:
...
def menu(message,tekUser):
    if tekUser == -1:
        ...
        tekUser = Kepper.find(message)
    key = types.InlineKeyboardMarkup()
    but_1 = types.InlineKeyboardButton(text="Авторизація",  callback_data="Автор")
...
def run_anketa(message: object, tekUser: object) -> object:
    ...
    key = types.InlineKeyboardMarkup()
    but_1 = types.InlineKeyboardButton(text="Так",  callback_data="Да")
    but_2 = types.InlineKeyboardButton(text="Ні",  callback_data="Нет")
    key.add(but_1, but_2)
    tekUser.NumQw = 1
...
def run_record(message: object, tekUser: object) -> object:
    if tekUser == -1:
        ...
        tekUser = Kepper.find(c.message)
    key = types.InlineKeyboardMarkup()
    but_1 = types.InlineKeyboardButton(text="Так",  callback_data="ДаЗапись")
    but_2 = types.InlineKeyboardButton(text="Ні",  callback_data="Нет")
    key.add(but_1, but_2)
    tekUser.NumQw = 1
...
def anather(message,tekUser):
    ...
    tekUser.Last_message = bot.send_message(message.chat.id, "Обери варіант:", reply_markup=key)
